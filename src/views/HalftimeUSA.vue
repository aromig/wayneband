  ...
  <div class="bg-white">
    ...
    <main class="relative pt-8 pb-12 bg-white">
      ...
      >
        ...
        <div id="form_facade" class="w-full min-h-full"></div>
      </section>
    </main>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async fetchData(page_slug) {
      return await api.getHalftimeUSAEvent(page_slug);
    },
    async fetchMedia(id) {
  ...
  async created() {
    ...
    if (this.page.form_facade_url != null) {
      let form_facade_script = document.createElement('script');
      form_facade_script.defer = true;
      form_facade_script.src = this.page.form_facade_url;
      document.getElementById('form_facade').appendChild(form_facade_script);
    }

    let page_title = this.page.title;

    document.title = `${page_title} - ${this.$route.meta.title}`;
